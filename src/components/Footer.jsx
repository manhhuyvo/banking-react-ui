              ...
              <div key={footerLink.key} className="flex-col ss:my-0 my-4 min-w-[150px]">
                ...
                <ul className='list-none'>
                  {footerLink.links.map(
                    (smallLink, index) => (
                      <div>
                        <li key={smallLink.index} className={`font-poppins font-normal text-[16px] leading-[24px] text-dimWhite hover:text-secondary curosr-pointer ${index !== footerLink.links.length - 1 ? 'mb-4' : 'mb-0'}`}>
                          <a href={smallLink.link}>{smallLink.name}</a>
                        </li>
                      </div>
                    )
                  )}
